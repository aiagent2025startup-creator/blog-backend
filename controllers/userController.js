...
exports.loginUser = async (req, res) => {
  try {
    ...
    // Configure cookie SameSite dynamically. For production with a separate
    // frontend origin (e.g., Vercel) we should use SameSite=None and Secure=true
    // so cookies can be sent on cross-site POST requests when the client uses
    // credentials: 'include' or axios withCredentials.
    ...
    // Also return token in response body for clients that prefer using
    // Authorization: Bearer <token> header (useful in local dev or cross-origin setups)
    // NOTE: Returning the token gives clients an option; for best security prefer
    // using the HttpOnly cookie with proper sameSite/CORS setup in production.
    res.status(200).json({
      success: true,
      message: 'Login successful',
      data: {
        user: safeUser,
        token, // convenience: can be used by frontend to set Authorization header
      },
    });
  } catch (error) {
    res.status(500).json({
...
exports.getCurrentUser = async (req, res) => {
  try {
    // `auth` middleware attaches req.user when token is valid
    ...
    res.status(500).json({ success: false, message: 'Error fetching current user', error: err.message });
  }
};
